...
<div class="row">
    <div class="container  col-11 mt-5 p-0 ">
        <div class="container col-10 mt-5 mb-4 pb-4" style="border:0.5px solid #C7C7CC;">
            ...
            <div class="form-group col-md-10 mx-2 type_military_id " style="display: none;" id="type_military_id">
               <div class="d-flex justify-content-end">
               <div class="radio-btns mx-md-4 ">
                    <input type="radio" class="form-check-input" id="police" name="type_military" value="police"
                        style="height:20px; width:20px;">
                    <label class="form-check-label mx-2" for="police">ضابط</label>
                </div>
                <div class="radio-btns mx-md-4">
                    <input type="radio" class="form-check-input" id="police_" name="type_military" value="police_"
                        style="height:20px; width:20px;" checked>
        ...
        <div class="form-row  mx-md-4 d-flex justify-content-center flex-row-reverse">
            ...
            <div class="form-group col-md-5 mx-2">
                <label for="input7"> <i class="fa-solid fa-asterisk" style="color:red; font-size:10px;"></i>
                    المهام</label>
                <select id="input7" name="rule_id" class="form-control select2" placeholder="المهام">
                    <option selected disabled>اختار من القائمة</option>
                    @foreach ($rule as $item)
                    <option value="{{ $item->id }}" {{ old('name') == $item->id ? 'selected' : '' }}> {{ $item->name }}
                    </option>
                    @endforeach
        ...
        <div class="form-group col-md-5 mx-2">
            ...
            <select id="region" name="region" class="form-control select2" placeholder="المنطقة">
                ...
                <option value="{{ $item->id }}" {{ old('region') == $item->id ? 'selected' : '' }}> {{ $item->name }}
                </option>
                @endforeach
            </select>
        </div>
...
<div class="form-row  mx-md-3 d-flex justify-content-center flex-row-reverse">
    ...
    <div class="form-group col-md-5 mx-2">
        ...
        <input type="text" id="input10" name="nationality" class="form-control" placeholder="الجنسية"
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(key, employee) {
                    ...
                    $('#Provinces').append('<option value="' + employee.id + '">' + employee
    ...
    if (Provinces_id) {
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(key, employee) {
                    console.log(employee);
                    $('#region').append('<option value="' + employee.id + '">' + employee
                        .name + '</option>');
