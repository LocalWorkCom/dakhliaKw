                    ...
                    <div class="form-group col-md-10">
                        ...
                        <select id="input25" name="department_id" class="form-control" placeholder="القسم" disabled>
                            ...
                            <option value="{{ $item->id }}"
                                {{ $rule_permission->department_id  == $item->id ? 'selected' : '' }}>{{ $item->name }}
                            </option>
                            @endforeach
                           ...
                           <div class="col-12 my-2">
                                    <div class="form-check">
                                        <input type="checkbox" id="exampleCheck{{ $item->id }}" value="{{ $item->id }}"
                                            style="width: 20px; height:20px; margin-left:1px; " name="permissions_ids[]"
                                ...
                                <div class="col-6 col-md-5 col-lg-4 my-2">
                                    <div class="form-check">
                                        <input type="checkbox" style="width: 20px; height:20px; margin-left:1px; "
                                            id="exampleCheck{{ $item->id }}" value="{{ $item->id }}"
                                            name="permissions_ids[]" class="form-check-input"
                                            {{ in_array($item->id, $hisPermissionIds) ? 'checked' : '' }} disabled>
                                        <label class="form-check-label m-1"
                                            for="exampleCheck{{ $item->id }}">{{__('permissions.'.$item->name)}}</label>
                                    </div>
                                </div>
