        ...
        <div class="row" dir="rtl">
            <div id="second-container" class="container moftsh col-11 mt-3 p-0 pb-3 hidden">
                ...
                <div class="form-row col-11 mb-2 mt-3 mx-md-2">
                    <div id="checkboxes-container" class="d-flex flex-wrap">
                        <!-- Checkboxes will be dynamically inserted here -->
                    </div>
                    {{-- <div class="form-group col-3 d-flex mx-md-4">
                        <input type="checkbox" name="pointIDs[]" value="{{ $point->id }}" id="pointIDs">
                        <label for="pointIDs">{{ $point->name }}</label>
                    </div> --}}
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (Array.isArray(data)) {
                        if (data.length > 0) {
                            // Data is not empty, create checkboxes
                            $.each(data, function(key, value) {
                                var checkboxHtml = '<div class="form-group col-3 d-flex mx-md-4">' +
                                    '<input type="checkbox" name="pointIDs[]" value="' + value.id + '" id="point_' + value.id + '">' +
                                    '<label for="point_' + value.id + '">' + value.name + '</label>' +
                                    '</div>';
                                checkboxesContainer.append(checkboxHtml);
                            });
                        } else {
                            // Data is an empty array, show a message
                            checkboxesContainer.append('<p>لا يوجد نقاط لهذه المحافظه بعد</p>');
                        }
                    } else {
                ...
                error: function(xhr) {
                    ...
                    $('#checkboxes-container').empty().append('<p>لا يوجد نقاط لهذه المحافظه بعد</p>');
                }
            });
        } else {
            $('#checkboxes-container').empty().append('<p>hojv h</p>');
        }
    });
});
