                ...
                @foreach ($inspectorGroups as $index => $inspectorGroup)
                    <div class="select-boxes mt-5 mx-4 col-10" dir="rtl">
                        <div class="check-one d-flex justify-content-start">
                            <input type="checkbox" class="toggle-radio-buttons mx-2" id="checkbox{{ $index }}" name="inspectors_ids[]" value="{{ $inspectorGroup['inspector_id']->id }}" {{ in_array($inspectorGroup['inspector_id']->id, old('inspectors_ids', $selectedInspectors)) ? 'checked' : '' }}>
