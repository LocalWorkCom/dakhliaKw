    ...
    <div class="row">
        <div class="container  col-11 mt-3 p-0  pt-5 pb-4">
            ...
            <div class="">
                ...
                <form action="{{ route('instant_mission.update', $IM->id) }}"
                    ...
                    <div class="container col-10 mt-4 p-4"
                        ...
                        <div
                            ...
                            <div class="form-group col-md-5 mx-2">
                                ...
                                <select id="group_team_id" name="group_team_id"
                                    ...
                                    @foreach ($groupTeams as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $IM->group_team_id == $item->id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-5 mx-2">
                                ...
                                <select id="inspectors" name="inspectors"
                                    ...
                                    @foreach ($inspectors as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $IM->inspector_id == $item->id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <input type="hidden" id="remaining-files"
                                name="remaining_files"
                                value="{{ $IM->attachment }}">
                            <div class="form-group col-md-10">
                                ...
                                <div class="file-preview1" id="file-preview1"
                                    dir="rtl"></div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const remainingFilesInput = document.getElementById(
            'remaining-files');
        ...
        filePreview.addEventListener('click', function(e) {
            if (e.target.classList.contains('delete-file')) {
                ...
                const fileName = e.target.getAttribute(
                    'data-file');

                console.log('File to be removed:', fileName);
                console.log('Current files:',
                    remainingFilesInput.value);

                const remainingFiles = remainingFilesInput.value
                    .split(',').filter(file => {
